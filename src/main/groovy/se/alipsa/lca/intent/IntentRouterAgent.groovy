...
class IntentRouterAgent {
  ...
  private String buildPrompt(String input, boolean strict) {
    ...
    builder.append("- /run: Execute a literal shell command the user explicitly asked to run (e.g. 'run the tests', 'run mvnw build')\n")
    ...
    builder.append("- /compact: Summarize and compact conversation history to free up context space\n\n")
    builder.append("Allowed commands: ").append(String.join(", ", allowedCommands)).append("\n\n")
    builder.append("IMPORTANT routing rules:\n")
    builder.append("- If the user explicitly says 'review', ALWAYS use /review. A code review includes identifying bugs, improvements, and suggestions — these are part of a review, not a plan.\n")
    builder.append("- Phrases like 'review this code', 'check for bugs', 'validate', 'look for issues', 'areas of improvement' → use /review\n")
    builder.append("- Phrases like 'suggestion for', 'how might', 'approach to', 'design for', 'propose' (WITHOUT the word 'review') → use /plan\n")
    ...
    builder.append("- Questions about git/branch status or open pull requests (e.g. 'is there a PR open?', 'what branch am I on?') → use /chat, NOT /run. /run is only for a literal command the user asked to execute.\n")
    builder.append("- Use /plan ONLY for design proposals, architectural plans, or implementation strategies where no existing code is being assessed.\n")
    builder.append("- For multi-line input, focus on the main request/question, not code examples or context.\n")
    builder.append("- If multiple intents are possible, choose the one matching the PRIMARY verb (review > plan > chat)\n")
